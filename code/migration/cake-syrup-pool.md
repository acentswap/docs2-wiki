---
description: Migrate to new ADE Syrup Pool
---

# ADE Syrup Pool

The new CakePool is a new $ADE staking contract built based on the CakeVault (the current auto ADE pool) and designed to work with AcentSwap MasterChef v2 to provide "stake $ADE, earn $ADE" functionality while offering more features such as fixed-term staking. The current Manual ADE pool will be retired after the migration.

The new CakePool will use a dummy token to harvest $ADE from MasterChef v2 and reward them to users who are staking $ADE. Users who lock their $ADE for longer will receive a more significant number of shares (boosted linearly based on duration), therefore, enjoy a higher yield.

### Do I need to migrate?&#x20;

If you are currently using `enterStaking` and `leaveStaking` on the AcentSwap MasterChef ([0x73feaa1eE314F8c655E354234017bE2193C9E24E](https://bscscan.com/address/0x73feaa1eE314F8c655E354234017bE2193C9E24E)), you will need to migrate to the new contract.
...
A 0.1% withdrawal fee will apply to the unstaking amount if you withdraw within 72 hours of the last deposit action. The withdrawal fee is cut from the final withdrawal amount before the ADE transfer.

### Overview

#### Deposit

If you are currently using the `enterStaking(uint256 _amount)` on the current AcentSwap MasterChef. You need to migrate to `deposit(uint256 _amount, uint256 _lockDuration)`. For flexible staking, simply use “0” as `_lockDuration`.

#### Staking Balance and Fees

```
Global variables: CakePoolContract // ADE pool contract
struct UserInfo {
    uint256 shares; // number of shares for a user.
    uint256 lastDepositedTime; // timestamp of the last deposit action
    uint256 cakeAtLastUserAction; // number of ADE at the last user action
    ...
    uint256 lockedAmount; // number of ADE locked at the start of the lock period.
}
```

**ADE staking amount (before subtracting all the fees)**
...
**ADE staking amount (after subtracting all the fees)**
...
However, you can query the number of ADE earned since the last action, using the difference between the current staking balance (mentioned above), and the number from `userInfo.cakeAtLastUserAction`.

#### Withdraw

If you are using the `leaveStaking(uint256 _amount)` method on the current AcentSwap MasterChef. You need to migrate to `withdraw(uint256 _shares)`.
...
```
// the number of ADE being withdrawn can be calculated by:
...
Please note that the final receiving amount will be affected by withdraw fee. If your function relies crucially on the final number of ADE being withdrawn, we recommend calculating that using the difference in ADE balance before and after the withdrawal action:

```
cakeBalPrev = ADE.balanceOf(address(this))
CakePool.withdraw(_sharesToWithdraw)
cakeBalNew = ADE.balanceOf(address(this))
cakeWithdrawn = cakeBalNew - cakeBalPrev
```

Or, calculate and subtract the withdraw fee when estimating the amount.

#### How to calculate the ADE per block distributed to the new ADE pool?

Previously, the manual ADE pool had a fixed 10 ADE/block emission. After migrating to MasterChef v2 and the new ADE pool, we can now adjust its emissions.

And here's how you can calculate the ADE per block distributed to the new ADE pool:
...
You can use the following testnet environment to test the integration of your project with the new AcentSwap ADE Pool. If you have any questions, please contact our team via the existing channels, or reach out to bun@pancakeswap.com via Email.
